...
| 运行时**返回值** | harness 拿 `output.schema` 校值（`additionalProperties: false`） | 操作其实成功了，模型收到的却是错误 |

第三行是最近才踩到的，前两道闸门全都放行：`console_connect` / `console_describe` 的返回体加了 `reused` / `openedBy`，声明却没跟上，于是**每一次**成功的 connect 都变成

```
"value.reused" is not a declared property (additionalProperties: false)
```

`register()` 只校 schema 不校值，`pnpm check:tools` 同理（它根本不看值），测试替身当时也只校两个 schema 的**形状**——三道闸门都绿。**未声明的字段不是被丢弃的多余数据，而是整个返回值不合格**，所以「body 加了字段、schema 忘了加」不是遗漏说明，是让那条工具彻底不可用。

对策：测试替身包住 `execute`，用 harness 自己的 `validateJsonSchemaValue(schema, value)` 校每一个返回值——用真实校验器就永远不会与该子集漂移；包在 `execute` 上而不是改各个调用点，任何用例都无法绕过。另加一条覆盖断言：断言的是**实际被校过值的工具集合**（而非已注册集合）与 12 个名字相等，因为 `console_clear` 注册了好几轮却没有任何用例调用过它，它的契约一次都没被看过。新增或遗漏工具时这条会立刻红。

配套：`required` 两种拼写的陷阱本身：
...
**但"已打开的会话与 options 解耦"这条规则本身有个例外，漏掉它就是一个 bug。** 上面那套规则是为**描述连接本身**的字段设计的：超时、编码、提示符/分页正则。改动它们去影响一条正在使用的连接，是让同一条连接前后语义不一致，所以必须冻结。而休眠策略（`dormantAutoWake` / `dormantProbeMs`）不描述连接，描述的是**用户正在编辑的一项策略**，而且承载它的两个控件就摆在实时控制台旁边。把它们一起冻结的后果是：面板显示新值，设备却按旧节奏继续收回车——用户报的"取消勾选还是会继续空闲保活""默认 120s 改成 10s 还是 120s"就是同一件事。修法是给 `ConsoleSession` 加一个 `adoptDormancyPolicy()` 窄接口，由 `PortManager.updateOptions` 在检测到这两个字段变化时推给所有已打开会话；**其余字段一律不推**，冻结保证原样保留（有一条测试专门断言"连接选项仍然冻结"，且验证过它在策略被整体转发时会失败）。

判据可以这样写：**"这个字段变了，是希望当前连接改行为，还是希望下次连接用新值？"** 超时/编码/正则是后者，保活间隔/总开关是前者。

## 5b. 一个勾选框就是一个承诺：别让两个字段共用一个开关

"空闲休眠自动唤醒"这个勾选框的 tooltip 同时描述了两件事——检测到标记补回车、以及定时保活；两句都挂在同一个"开启后"下面。但实现里 `dormantAutoWake` 和 `dormantProbeMs` 是两个独立字段，取消勾选只停了前者，保活仍旧按时发回车。用户读到的承诺（一个开关管住所有自动回车）和代码的行为（两个独立旋钮）不一致，于是"取消勾选还是继续保活"。

修法是把勾选框做成**总开关**：`armKeepalive()` 同时检查 `dormantAutoWake`。关键细节是**不要**把秒数清零来实现——`dormantProbeMs` 原样保留，重新勾选即按原间隔恢复，用户不需要重新输入。面板上在总开关关闭时补一句"（已停止自动回车）"说明秒数仍在、只是暂不生效，否则"关闭开关但秒数还显示 120"本身就是新的困惑源。

两个 UI 细节：这句话必须放在 `<label>` **外面**——label 内的文本会成为控件的 accessible name，放进去会把 `空闲休眠自动唤醒` 改名成 `空闲休眠自动唤醒（已停止自动回车）`，所有按 label 查找的测试与无障碍工具一起失效（已被测试覆盖）。
...
- 面板只呈现这个框**真正能编辑**的东西：`fenceRules` 一行一条，外加「未命中时用兜底策略」这一句。遗留字段 `highRiskPatterns` 仍由宿主honour（`tests/guard.spec.ts` 钉住），但它不归这个框管，所以面板不再为它显示一段只读说明——面板上出现的每个东西都应当是可改的，否则读的人会把说明当成能编辑的规则。`tests/fence-editor.spec.tsx` 相应改成断言「以 `fenceRules` 为界」：盒子为空、兜底策略仍被说明、且遗留 pattern 不出现在 DOM 里。
- 放在 `<label>` 里的文字会成为控件的 accessible name。给一个已有 label 的勾选框补一句状态提示（"（已停止自动回车）"）时若写在 label 内，会把 `空闲休眠自动唤醒` 改名成 `空闲休眠自动唤醒（已停止自动回车）`，按 label 查找的代码与屏幕阅读器一起失效。状态提示属于这**对**控件，放在 label 外并加 `data-*` 钩子。
